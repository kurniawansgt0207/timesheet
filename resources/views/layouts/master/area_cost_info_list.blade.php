            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table id="example2" class="table table-bordered table-hover">
                        ...
                        <tbody>
                        @foreach($area_cost_info as $area_cost=>$p)
                            <tr>
                                <td align="center">{{$area_cost+1}}</td>
                                <td align="center">{{$p->area}}</td>
                                <td align="left">{{$p->costname}}</td>
                                <td align="right">{{ number_format($p->costAmt) }}</td>
                                <td align="center"><?php echo ($p->isActive==1) ? "Aktif" : "Tidak Aktif";?></td>
                                <td align="center">
                                <a href="{{url('/')}}/master/areacost/edit_data/{{ $p->id }}">
...
<script>
    function addData(){
        window.location.href="{{url('/master/areacost/add_data')}}";
    }

    function deleteData(id){
      var tny = confirm("Yakin Akan Menghapus Data Ini ?"+id);
      if(tny == 1){
        window.location.href = "{{url('/')}}/api/master/areacost/delete/"+id;
      }
    }
</script>
@endsection
