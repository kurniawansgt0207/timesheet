            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table id="example2" class="table table-bordered table-hover">
                        ...
                        <tbody>
                        @foreach($user_info as $user=>$p)
                            <tr>
                                <td>{{$user+1}}</td>
                                <td>{{$p->name}}</td>
                                <td>{{$p->email}}</td>
                                <td>{{ ($p->stat_active==1) ? "Aktif" : "Tidak Aktif" }}</td>
                                <td align="center">
                                <a href="{{url('/')}}/master/user/edit_data/{{ $p->id }}">
...
<script>
    function addData(){
        window.location.href="{{url('/master/user/add_data')}}";
    }

    function deleteData(id){
      var tny = confirm("Yakin Akan Menghapus Data Ini ?"+id);
      if(tny == 1){
        window.location.href = "{{url('/')}}/api/master/user/delete/"+id;
      }
    }
</script>
@endsection
