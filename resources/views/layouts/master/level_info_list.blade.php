            ...
            <div class="box">
                ...
                <div class="box-body">
                    <table id="example2" class="table table-bordered table-hover">
                        ...
                        <tbody>
                        @foreach($level_info as $level=>$p)
                            <tr>
                                <td>{{$level+1}}</td>
                                <td>{{$p->levelname}}</td>
                                <td>{{$p->levelno}}</td>
                                <td align="center">
                                <a href="{{url('/')}}/master/level/edit_data/{{ $p->id }}">
...
<script>
    function addData(){
        window.location.href="{{url('/master/level/add_data')}}";
    }

    function deleteData(id){
      var tny = confirm("Yakin Akan Menghapus Data Ini ?"+id);
      if(tny == 1){
        window.location.href = "{{url('/')}}/api/master/level/delete/"+id;
      }
    }
</script>
@endsection
